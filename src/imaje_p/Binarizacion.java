...
public class Binarizacion {
    ...
    public static Image binarizarImagen (Image original, int j){
        BufferedImage bufim = HerramientasImagen.toBufferedImage(original);
        
        for(int x=0; x < bufim.getWidth(null);x++){
            for(int y=0; y<bufim.getHeight(null);y++){
                Color col = new Color(bufim.getRGB(x, y));
                
                col = new Color(validar(col.getRed(),j),validar(col.getGreen(),j),validar(col.getBlue(),j));
                bufim.setRGB(x, y, col.getRGB());
            }
        }
        original = HerramientasImagen.toImage(bufim);
        return original;
    }
    public static Image binarizarImagen (Image original, int j, int j2){
        ...
        for(int x=0; x < bufim.getWidth(null);x++){
            for(int y=0; y<bufim.getHeight(null);y++){
                ...
                col = new Color(validar(col.getRed(),j,j2),validar(col.getGreen(),j,j2),validar(col.getBlue(),j,j2));
                bufim.setRGB(x, y, col.getRGB());
            }
        }
        original = HerramientasImagen.toImage(bufim);
    ...
    private static int validar(int color, int j) {
        if (color >= j) return 255;
        if (color < j) return 0;
        return color;
    }
    
    private static int validar(int color, int j, int j2) {
        if (color <= j||color >=j2) return 255;
        else return 0;
